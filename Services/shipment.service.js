...
module.exports = {
  ...
  getAllShipments: (callBack) => {
    pool.query(
        "SELECT shipment.*, supplier.name AS supplier_name FROM shipment LEFT JOIN supplier ON shipment.supplier_id = supplier.id",
        (error, results) => {
            if (error) {
                return callBack(error);
            }
  ...
  getShipmentsById: (model, callBack) => {
    pool.query(
        "SELECT shipment.*, supplier.name AS supplier_name FROM shipment LEFT JOIN supplier ON shipment.supplier_id = supplier.id WHERE shipment.id LIKE ?",
        [`%${model.id}%`],
        (error, results) => {
            if (error) {
                return callBack(error);
